...
* **LLM-Based Brand Detection & Grounding**: Brand classification has transitioned from a fixed whitelist to an LLM-based detection function, `getProductBrandIfBranded` in [production-bible-agent.ts:13-97](file:///c:/Users/Admin/Desktop/YT_Prompt.ai/backend/src/agents/production-bible-agent.ts#L13-L97). It checks a keyword fast-path (e.g., *sting*, *coke*) before invoking Gemini via `GeminiService.generateJSON` with a flexible response schema to determine if the topic/title centers on any commercial product. If branded, a single grounded research query is made via `GeminiService.generateGroundedText` (using the `googleSearch` tool) in [production-bible-agent.ts:229-232](file:///c:/Users/Admin/Desktop/YT_Prompt.ai/backend/src/agents/production-bible-agent.ts#L229-L232), generalizing to any real-world brand (including regional/obscure ones).
* **Pipeline Propagation & Pre-Generation Evaluation**: The brand status propagates from the `object_registry` into `objects_featured` in [veoprompts.routes.ts:189-199](file:///c:/Users/Admin/Desktop/YT_Prompt.ai/backend/src/routes/veoprompts.routes.ts#L189-L199) (preserving the object's `id`, `object_id`, and `is_branded_product`). The resolved scene assembled in [veoprompts.routes.ts:203-216](file:///c:/Users/Admin/Desktop/YT_Prompt.ai/backend/src/routes/veoprompts.routes.ts#L203-L216) includes `raw_json` which allows `veoAgent.run` to compute `hasBrandedProductFeatured` pre-generation. This prevents standard brand/logo/text exclusions (`BRAND_AVOIDS`) from being passed to the generator's instructions, ensuring the brand description is generated woven-in rather than bolted on as a post-process prop lock.
...
* **Regression Harness**: The script [branding-link-check.ts](file:///c:/Users/Admin/Desktop/YT_Prompt.ai/backend/src/test/branding-link-check.ts) acts as a standing E2E regression check. It executes the real agent chain (Bible → Scene → Veo) for two fixtures (branded "Sting energy drink" vs generic "container ship") and runs a 6-point verification for each:
  1. *Bible*: `is_branded_product` matches expectations; visual lock contains brand cues / is clean.
  2. *Scene*: resolved `objects_featured` still carries the `is_branded_product` flag.
  3. *Veo*: `hasBrandedProductFeatured` evaluates correctly both initially and from database.
  4. *Veo Avoid*: `BRAND_AVOIDS` are suppressed for branded products and included for generics.
  5. *Veo Visual*: visual descriptions correctly incorporate branded cues or stay clean.
  6. *Isolation*: overlay suggestions do not leak into the prompt text.
...
### 8.6 Veo Prompt Realism & Composition Guardrails
To enforce concrete, physical visual generation and prevent stylistic or technical meta-jargon from contaminating the video generation prompts, the pipeline implements several layer-specific rules and post-process guardrails:

* **Adjective-Variety Guardrail**:
  - **Prompt-Level**: In [veo.prompt.ts:L12-L14](file:///c:/Users/Admin/Desktop/YT_Prompt.ai/backend/src/prompts/veo.prompt.ts#L12-L14), a strict observational realism rule (`adjectiveVarietyRule`) is gated by profile. If `profile.id === 'documentary'`, all evaluative or filler adjectives (e.g., "beautiful", "stunning", "majestic") are banned, requiring concrete observable nouns, materials, and actions instead.
  - **Programmatic Backstop**: In [veo-agent.ts:L1556](file:///c:/Users/Admin/Desktop/YT_Prompt.ai/backend/src/agents/veo-agent.ts#L1556), the post-processor scrubs evaluative adjectives and their common adverb forms (e.g., *beautifully*, *majestically*, *dramatically*) across all profiles before outputting the final prompt.
* **Word-Budget Enforcement**:
  - **Visual Prompt Constraints**: The `visual` description field targets 60–75 words, and is strictly capped between 40 and 80 words (validated in [veo-validation.ts:L45-L49](file:///c:/Users/Admin/Desktop/YT_Prompt.ai/backend/src/utils/veo-validation.ts#L45-L49)).
  - **Condense Retry**: If the generated `visual` exceeds 80 words, [veo-agent.ts:L901-L904](file:///c:/Users/Admin/Desktop/YT_Prompt.ai/backend/src/agents/veo-agent.ts#L901-L904) triggers a single condense-retry using a specific rewrite prompt instruction, demanding the model prune secondary activities while maintaining the single dominant action.
  - **Sentence-Boundary Trim**: If the prompt still exceeds 80 words after retry, a backstop at [veo-agent.ts:L1440-L1460](file:///c:/Users/Admin/Desktop/YT_Prompt.ai/backend/src/agents/veo-agent.ts#L1440-L1460) slices trailing full sentences until the description falls within the 40–80 word budget, ensuring the prompt ends on a valid sentence boundary (period).
  - **One-Dominant-Action**: Reinforces exactly one continuous action beat without temporal jumps or transition instructions, as defined in the system prompt rules ([veo.prompt.ts:L111](file:///c:/Users/Admin/Desktop/YT_Prompt.ai/backend/src/prompts/veo.prompt.ts#L111)).
* **Meta-Jargon Handling**:
  - **Absolute Word Ban**: Prompt rules in [veo.prompt.ts:L114](file:///c:/Users/Admin/Desktop/YT_Prompt.ai/backend/src/prompts/veo.prompt.ts#L114) ban rendering and metadata jargon (e.g., *HDR*, *depth of field*, *bokeh*, *lens flare*, *focus*, *textures*, *fidelity*, *film grain*) in the `visual` and `shot` description fields.
  - **Post-Process Scrub**: In [veo-agent.ts:L1558](file:///c:/Users/Admin/Desktop/YT_Prompt.ai/backend/src/agents/veo-agent.ts#L1558), a programmatic regex cleanup prunes metadata filler terms from `data.visual` while preserving valid lighting and lens parameters in their dedicated schema fields.
* **Environmental Realism & Micro-Physics**:
  - Gated by profile, `environmentalRealismRule` in [veo.prompt.ts:L16-L18](file:///c:/Users/Admin/Desktop/YT_Prompt.ai/backend/src/prompts/veo.prompt.ts#L16-L18) instructs the model to describe particulate/atmosphere, material wear, light surface interaction, and physical motion mechanics. It enforces word-neutrality by substituting generic visual filler with specific physical details.
* **Composition & Framing (Shot Field)**:
  - Guided by `shotCompositionRule` in [veo.prompt.ts:L20-L28](file:///c:/Users/Admin/Desktop/YT_Prompt.ai/backend/src/prompts/veo.prompt.ts#L20-L28) (mapped to the JSON `"shot"` field in [veo.prompt.ts:L189](file:///c:/Users/Admin/Desktop/YT_Prompt.ai/backend/src/prompts/veo.prompt.ts#L189)), the model generates a single concise sentence describing framing conventions (e.g. headroom, eyeline, height, rule-of-thirds) and physical depth layering (clear foreground, midground, background).
  - **Separation of Concerns**: The camera framing and layering intent are restricted to the `shot` field, leaving the physical scene action and elements to the `visual` field, preventing duplication and keeping a single coherent shot type.
